...
export function StaticTextWarning() {
    ...
    return (
        <div className="flex items-start gap-2 rounded-md bg-amber-50 p-2 text-xs text-amber-700 border border-amber-200">
            ...
            <span>
                {t('flow.textOrAudioInput.staticTextWarning')}
            </span>
        </div>
    );
}
        ...
        <>
            ...
            >
                <div className="flex items-center gap-2">
                    ...
                    <Label htmlFor="toa-text" className="font-normal cursor-pointer">{t('flow.textOrAudioInput.text')}</Label>
                </div>
                <div className="flex items-center gap-2">
                    <RadioGroupItem value="audio" id="toa-audio" />
                    <Label htmlFor="toa-audio" className="font-normal cursor-pointer">{t('flow.textOrAudioInput.audio')}</Label>
                </div>
            </RadioGroup>
            {type === 'text' ? (
                children
    ...
    return (
        <div className="space-y-2">
            <Label className="text-xs text-muted-foreground">
                {t('flow.textOrAudioInput.selectRecording')}
            </Label>
                ...
                <PopoverTrigger asChild>
                    ...
                    >
                        {selected ? (
                            ...
                            <span className="text-muted-foreground">{t('flow.textOrAudioInput.placeholder')}</span>
                        )}
                ...
                >
                    {recordings.length === 0 ? (
                        <div className="p-3 text-sm text-muted-foreground text-center">
                            {t('flow.textOrAudioInput.noRecordings')}
                        </div>
                    ) : (
                        <div>
                            <div className="p-2 border-b">
                                <div className="relative">
                                    ...
                                    <Input
                                        placeholder={t('flow.textOrAudioInput.searchPlaceholder')}
                                        value={search}
                            ...
                            {filtered.length === 0 ? (
                                <div className="p-3 text-sm text-muted-foreground text-center">
                                    {t('flow.textOrAudioInput.noMatch', { query: search })}
                                </div>
                            ) : filtered.map((r) => {
