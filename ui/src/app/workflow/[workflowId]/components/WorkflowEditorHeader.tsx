...
export const WorkflowEditorHeader = ({
    ...
    const handlePublish = async () => {
        ...
        toast.promise(promise, {
            loading: t("workflow.editor.publishing"),
            success: t("workflow.editor.publishSuccess"),
            error: t("workflow.editor.publishFailed"),
        });
        try {
            await promise;
            onPublished();
    ...
    const handleDuplicate = async () => {
        ...
        toast.promise(promise, {
            loading: t("workflow.editor.duplicating"),
            success: t("workflow.editor.duplicateSuccess"),
            error: t("workflow.editor.duplicateFailed"),
        });
        try {
    ...
    const handleCopyAgentUuid = async () => {
        if (!workflowUuid) {
            toast.error(t("workflow.editor.agentUuidNotAvailable"));
            return;
        }
        try {
            await navigator.clipboard.writeText(workflowUuid);
            toast.success(t("workflow.editor.agentUuidCopied"));
        } catch {
            toast.error(t("workflow.editor.copyAgentUuidFailed"));
        }
    };
    ...
    const attemptSave = async () => {
        ...
        if (trimmed.length === 0) {
            setRename({ ...rename, error: t("workflow.editor.nameCannotBeEmpty") });
            return;
        }
        ...
        try {
            ...
            toast.error(t("workflow.editor.renameFailed"));
        ...
        <div className="flex items-center justify-between w-full h-14 px-4 bg-[#1a1a1a] border-b border-[#2a2a2a]">
            ...
            <div className="flex items-center gap-3 mr-4">
                <button
                    ...
                    aria-label={t("workflow.editor.openMenu")}
                >
                        ...
                        <>
                            ...
                            {!isViewingHistoricalVersion && (
                                <button
                                    ...
                                    aria-label={t("workflow.editor.renameWorkflow")}
                                    className="flex items-center justify-center w-8 h-8 rounded-lg hover:bg-[#2a2a2a] transition-colors"
                                >
            ...
            <div className="flex items-center gap-3">
                ...
                {isViewingHistoricalVersion && (
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-md border border-blue-500/30 bg-blue-500/10">
                        ...
                        <span className="text-sm text-blue-400">
                            {t("workflow.editor.viewingVersion", { version: activeVersionLabel ?? "" })}
                        </span>
                    </div>
                )}
                ...
                {isDirty && !isViewingHistoricalVersion && (
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-md border border-yellow-500/30 bg-yellow-500/10">
                        ...
                        <span className="text-sm text-yellow-500">{t("workflow.editor.unsavedChanges")}</span>
                    </div>
                )}
                    ...
                    <Popover>
                        <PopoverTrigger asChild>
                            <button className="flex items-center gap-2 px-3 py-1.5 rounded-md border border-red-500/30 bg-red-500/10 hover:bg-red-500/20 transition-colors cursor-pointer">
                                ...
                                <span className="text-sm text-red-500">
                                    {t("workflow.editor.errorCount", { count: workflowValidationErrors.length })}
                                </span>
                            </button>
                        </PopoverTrigger>
                        <PopoverContent
                        ...
                        >
                            <div className="px-4 py-3 border-b border-[#3a3a3a]">
                                <h3 className="text-sm font-medium text-white">{t("workflow.editor.validationErrors")}</h3>
                            </div>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                {(error.kind === "node" || error.kind === "edge") && error.id && (
                                                    <p className="text-xs text-gray-400 mb-1">
                                                        {t(error.kind === "node" ? "workflow.editor.node" : "workflow.editor.edge")}: {error.id}
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="bg-[#1a1a1a] border-[#3a3a3a]">
                        ...
                        >
                            ...
                            {duplicating ? t("workflow.editor.duplicatingShort") : t("workflow.editor.duplicateWorkflow")}
                        </DropdownMenuItem>
                        <DropdownMenuItem
                        ...
                        >
                            ...
                            {t("workflow.editor.downloadWorkflow")}
                        </DropdownMenuItem>
                        <DropdownMenuItem
                        ...
                        >
                            ...
                            {t("workflow.editor.copyAgentUuid")}
                        </DropdownMenuItem>
                    </DropdownMenuContent>
                </DropdownMenu>
