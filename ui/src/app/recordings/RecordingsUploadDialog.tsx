    ...
    const transcribeFile = async (pendingId: string, file: File) => {
        ...
        try {
            ...
            setPendingFiles((prev) =>
                prev.map((p) =>
                    p.id === pendingId
                        ? { ...p, isTranscribing: false, error: t('recordings.upload.autoTranscribeFailed') }
                        : p
                )
            );
        }
    ...
    const addPendingFiles = (files: File[]) => {
        ...
        for (const file of files) {
            if (file.size > MAX_FILE_SIZE) {
                setError(t('recordings.upload.fileTooLarge', { name: file.name, size: (file.size / (1024 * 1024)).toFixed(1) }));
                continue;
            }
        ...
        try {
            ...
            mediaRecorder.onstop = () => {
                ...
                if (blob.size > MAX_FILE_SIZE) {
                    setError(t('recordings.upload.recordingTooLarge', { size: (blob.size / (1024 * 1024)).toFixed(1) }));
                    resetRecordingState();
                    return;
                }
            ...
            setError(t('recordings.upload.micDenied'));
            resetRecordingState();
        }
    };
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (!uploadUrlResponse.data?.items) {
                throw new Error(t('recordings.upload.errors.getUploadUrls'));
            }
            ...
            await Promise.all(
                items.map(async (item: RecordingUploadResponseSchema, idx: number) => {
                    ...
                    if (!uploadResponse.ok) {
                        throw new Error(t('recordings.upload.errors.fileUploadFailed', { name: file.name }));
                    }
                })
            );
            ...
            setError(err instanceof Error ? err.message : t('recordings.upload.errors.uploadFailed'));
        } finally {
            setUploading(false);
        }
    };
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>{t('recordings.upload.title')}</DialogTitle>
                    <DialogDescription>
                        {t('recordings.upload.description')}
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-3">
                    ...
                    <div>
                        <Label className="text-xs text-muted-foreground">{t('recordings.upload.audioFiles')}</Label>
                        <div className="flex gap-2">
                            ...
                            >
                                ...
                                <span className="text-muted-foreground">{t('recordings.upload.chooseFiles')}</span>
                            </Button>
                            {recordingStep === "idle" && (
                                <Button
                                    type="button"
                    ...
                    {(recordingStep === "naming" || isRecording) && (
                        <div className="space-y-2 rounded-md border border-dashed p-3 bg-muted/20">
                            {recordingStep === "naming" && (
                                <>
                                    <div>
                                        <Label className="text-xs text-muted-foreground">{t('recordings.upload.recordingName')}</Label>
                                        <Input
                                            placeholder={t('recordings.upload.recordingNamePlaceholder')}
                                    ...
                                    <div className="flex gap-2">
                                        <Button size="sm" onClick={startRecording} disabled={!recordingFilename.trim()}>
                                            <Mic className="w-4 h-4 mr-1" />
                                            {t('recordings.upload.startRecording')}
                                        </Button>
                    ...
                    {pendingFiles.length > 0 && (
                        <div className="space-y-2">
                            <Label className="text-xs text-muted-foreground">
                                {t('recordings.upload.pending', { count: pendingFiles.length })}
                            </Label>
                            {pendingFiles.map((pf) => (
                                <div key={pf.id} className="rounded-md border p-2 space-y-1.5 bg-muted/10">
                                    ...
                                    <Textarea
                                        placeholder={pf.isTranscribing ? t('recordings.upload.transcribing') : t('recordings.upload.transcriptPlaceholder')}
                                        value={pf.transcript}
                    ...
                    {/* Language */}
                    <div>
                        <Label className="text-xs text-muted-foreground">{t('recordings.upload.language')}</Label>
                    ...
                    >
                        ...
                        {uploading
                            ? t('recordings.upload.uploading')
                            : t('recordings.upload.uploadButton', { count: readyCount })}
                    </Button>
                </div>
            </DialogContent>
        </Dialog>
