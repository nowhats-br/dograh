...
export default function DocumentList({ refreshTrigger }: DocumentListProps) {
  ...
  const fetchDocuments = useCallback(async () => {
    try {
      ...
      if (response.error || !response.data) {
        throw new Error(t('files.list.errors.fetchFailed'));
      }

      setDocuments(response.data.documents);
    } catch (err) {
      setError(err instanceof Error ? err.message : t('files.list.errors.fetchFailed'));
  ...
  const handleDelete = async (documentUuid: string, filename: string) => {
    if (!confirm(t('files.list.deleteConfirm', { filename }))) return;
    ...
    try {
      ...
      if (response.error) {
        throw new Error(t('files.list.errors.deleteFailed'));
      }

      toast.success(t('files.list.deleted', { filename }));
      fetchDocuments();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : t('files.list.errors.deleteFailed'));
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'completed':
        return <Badge className="bg-green-500">{t('files.list.status.completed')}</Badge>;
      case 'processing':
        return (
          <Badge variant="secondary" className="animate-pulse">
            {t('files.list.status.processing')}
          </Badge>
        );
      case 'pending':
        return <Badge variant="outline">{t('files.list.status.pending')}</Badge>;
      case 'failed':
        return <Badge variant="destructive">{t('files.list.status.failed')}</Badge>;
      default:
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {doc.processing_error && (
                    <p className="text-xs text-destructive mt-1">
                      {t('files.list.error')}: {doc.processing_error}
                    </p>
                  )}
